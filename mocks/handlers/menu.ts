...
export const menuHandlers = [
  ...
  // 获取菜单详情
  http.get("/api/v1/sys/menu/detail", ({ request }) => {
    const url = new URL(request.url)
  ...
  // 更新菜单
  http.put("/api/v1/sys/menu/update", async ({ request }) => {
    const menuData = (await request.json()) as Menu
  ...
  // 删除菜单
  http.delete("/api/v1/sys/menu/delete", ({ request }) => {
    const url = new URL(request.url)
    const id = Number(url.searchParams.get("id"))
