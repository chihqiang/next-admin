...
export async function menuDetailApi(id: number): Promise<Menu> {
  return await request.get<Menu>(`/api/v1/sys/menu/detail`, { params: { id } })
}
// 创建菜单
...
export async function menuUpdateApi(data: Menu): Promise<Menu> {
  return await request.put<Menu>(`/api/v1/sys/menu/update`, data)
}
// 删除菜单
export async function menuDeleteApi(id: number): Promise<Menu> {
  return await request.delete<Menu>(`/api/v1/sys/menu/delete`, {
    params: { id },
  })
}
