...
export async function accountDetailApi(id: number): Promise<Account> {
  return await request.get<Account>(`/api/v1/sys/account/detail`, {
    params: { id },
  })
}
...
export async function accountUpdateApi(
  ...
  return await request.put<Account>(`/api/v1/sys/account/update`, data)
}

// 删除账号
export async function accountDeleteApi(id: number): Promise<Account> {
  return await request.delete<Account>(`/api/v1/sys/account/delete`, {
    params: { id },
  })
}
